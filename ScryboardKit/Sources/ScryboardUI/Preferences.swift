...
public enum CardSize: String, Sendable, Hashable, CaseIterable, Codable {
    ...
    /// Large cells outgrow Scryfall's `small` scan (146 px wide) by three
    /// times on a 3× screen, so they load the `normal` scan instead. It costs
    /// more bytes per card; the decoded thumbnail is still capped at cell size.
    public var scan: ImageSize {
        self == .large ? .normal : .small
    }
